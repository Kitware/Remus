...
//------------------------------------------------------------------------------
void terminate_job(remus::proto::Job job,
...
//------------------------------------------------------------------------------
void verify_job_is_terminated(boost::shared_ptr<remus::Worker> worker,
                              remus::worker::Job job)
{
  //wait for the server to send terminate job msg to the worker
  while( !worker->jobShouldBeTerminated( job ) )
    { remus::common::SleepForMillisec(50); }

  const std::size_t numPendingJobs = worker->pendingJobCount();
  REMUS_ASSERT( (numPendingJobs==0) )
}
...
int TerminateRunningJob(int argc, char* argv[])
{
  ...
  //now the job is 'active' have the client call terminate_job
  terminate_job(clientJob, client);

  //verify that the worker API states the job should be terminated
  verify_job_is_terminated(worker, workerJob);
