                              ...
                              boost::shared_ptr< remus::Client > client)
{
  ...
  std::cout << "Total milliseconds: " << dur.total_milliseconds() << std::endl;
  ...
  std::cout << "Messages sent & recv per millisec " << message_pairs_per_msec  << std::endl;

  const boost::int64_t bytes_sent_per_msec = total_bytes_sent / dur.total_milliseconds();
  const boost::int64_t bytes_recv_per_msec = total_bytes_recv / dur.total_milliseconds();

  std::cout << "Bandwidth sent per byte/msec: " << bytes_sent_per_msec << std::endl;
  std::cout << "Bandwidth recv per byte/msec: " << bytes_recv_per_msec << std::endl;

  //This is to make sure that our baseline performance doesn't degrade below
  //a certain level
#if defined(__APPLE__)
  REMUS_ASSERT( (bytes_sent_per_msec >= 275))
  REMUS_ASSERT( (bytes_recv_per_msec >= 2750))
#elif defined(_WIN32)
  REMUS_ASSERT( (bytes_sent_per_msec >= 275))
  REMUS_ASSERT( (bytes_recv_per_msec >= 2750))
#else
  //unix just does better with loopback
  REMUS_ASSERT( (bytes_sent_per_msec >= 375))
  REMUS_ASSERT( (bytes_recv_per_msec >= 3750))
#endif
...
#if defined(__APPLE__)
  REMUS_ASSERT( (bytes_sent_per_msec <= 550))
  REMUS_ASSERT( (bytes_recv_per_msec <= 5500))
#elif defined(_WIN32)
  REMUS_ASSERT( (bytes_sent_per_msec <= 550))
  REMUS_ASSERT( (bytes_recv_per_msec <= 5500))
#else
  //unix just does better with loopback
  REMUS_ASSERT( (bytes_sent_per_msec <= 900))
  REMUS_ASSERT( (bytes_recv_per_msec <= 9000))
#endif
